class ExportContentService < ApplicationService
  attr_accessor :args, :user
  def initialize(args)
    @args = args.with_indifferent_access
    RequestStore.store[:user_id] = args[:user_id]
    @user = RailsVger::AuthApi::User.api_show(args[:user_id])
  end
  ...
  def call
    ...
    file_path = "/tmp/journey_export_#{journey_id}_#{Time.now.to_i}.xlsx"
    ...
    AnalyticsMailer.send_report({
      subject: "Content Exports #{journey.name}",
      body: "Export for all contents for #{journey.name} is attached",
      time_zone: user.try(:time_zone),
      attachments: {
        files: [{
          name: "Journey content.xlsx",
          path: file_path
        }]
      },
...
ExportContentService.call({journey_id: '683622004be8090008bb613d', user_id: '66f3a6266ac049ff993c0b59'})